...
Bright Harbor Careers uses Supabase for durable job, application, account request, job board layout, note, scorecard, and role data.
...
The migrations create applicant profiles automatically for new Supabase Auth users and add a public `job_board_settings` record for the editable applicant-board header. The public career site can read `published` jobs and submit fresh applications. Hiring-team visitors can request access, which creates an `account_requests` row and emails `hr@brightharbor.org` approval links through Netlify functions. HR users sign in from the Hiring Team login page with their work email as the username and their Supabase password; row-level security decides whether that signed-in user can create jobs, update applications, manage profiles, or edit the job board layout.
